...
export default function Home() {
  ...
  const pickWinnerHandler = async () => {
    ...
    try {
      ...
      setSuccessMsg(`The winner is ${address}. Remember to click "Pay Winner" in order for pot funds to be transferred to their address`)
    } catch(err) {
      setError(err.message)
    }
  }
  ...
  const payWinnerHandler = async () => {
    ...
    try {
      ...
      setSuccessMsg(`The pot is now transferred to ${winnerAddress}`)
      updateState()
    } catch(err) {
      setError(err.message)
    }
          ...
          <section className="mt-5">
            <div className="columns">
              <div className="column is-two-thirds">
                <section className="mt-5">
                  <p>Cost of admission into the lottery is 0.001 ETH</p>
                  <p>The chosen winner will be transferred all funds entered into the lottery pot</p>
              ...
              <section className="mt-5">
                ...
                <section className="mt-5">
                  <div className="card">
                    <div className="card-content">
                      <div className="content">
                        <h2>Players in Lottery #{lotteryId}</h2>
                        <ul className="ml-0">
                          {
                          ...
                          (lotteryHistory && lotteryHistory.length > 0) && lotteryHistory.map(item => {
                            if (lotteryId != item.id) {
                              return <div className="history-entry mt-3" key={item.id}>
                                <div>The winner of lottery #{item.id} was:</div>
                                <div>
                                  <a href={`https://goerli.etherscan.io/address/${item.address}`} target="_blank">
                                    {item.address}
                                  </a>
                                </div>
                              </div>
